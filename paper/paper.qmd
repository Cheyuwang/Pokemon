...
I analysis the data in R(R Core Team (2023)), with additional tools for support the analysis, including tidyverse(Wickham et al. 2019), here(Müller 2020), dplyr(Wickham et
al. 2023), readr(Wickham, Hester, and Bryan 2023), modelsummary(Arel-Bundock 2022),
janitor(Firke 2023), tibble(Müller and Wickham 2023), ggplot2(Wickham 2016), rstanarm (Goodrich et al. 2020) and research method is from Alexander (2023).
...
In @tbl-2, The Poisson regression analysis looks at the relationship between the number of yellow cards issued to the home team (HY) and the number of goals scored by the opposing team. According to the findings, as the number of home yellow cards increases, so does the expected log count of away goals, but only slightly and not significantly.However,we have our results that shows Poisson regression model is used to calculate the number of Full-Time Away Goals (FTAG) in football matches, with Home Yellow Cards serving as the predictor variable. The model estimates the intercept (0.30054) and the HY coefficient (0.04215), which represent the expected change in the log count of away goals for each additional home yellow card issued. However, the relationship between home yellow cards and away goals is not statistically significant, implying that yellow cards have no strong or consistent effect on the number of goals scored by the away team. The AIC (969.53) and the small difference between the null and residual deviance indicate that including the number of home yellow cards has no significant impact on the model's predictive capability. The model's summary indicates that other factors may better explain the variability in away goals, or that the effect of yellow cards is too subtle to detect in this dataset.

```{r}
#| label: tbl-2
#| fig-cap: Kabble for poisson regresion 
#| echo: false
# Fit the model
